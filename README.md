...
Every series resolves through a three-tier ladder, and the figure shows which
tier produced the numbers on screen:

| Badge | Meaning |
|---|---|
| **Live** | The reader's browser called the publisher's API directly and it answered. |
...
| **Repo snapshot** | The live call failed, so the committed fallback is shown — dated in the badge. |

26 of the 60 series are API-backed and re-fetch themselves on every page load
(World Bank Indicators, plus IMF and data.gov.in providers). The rest come from
publishers who release PDFs — the National Crime Records Bureau, the Ministry of
Defence, Union Budget documents — and are transcribed with a citation.

### Status of the committed snapshot

> The snapshot in this initial release was **transcribed by hand** from published
> figures. It has not been generated by calling the APIs, because the environment
> this repository was built in had no outbound network access.
>
> - The 26 API-backed series correct themselves the moment a reader loads the
>   page with working network access.
